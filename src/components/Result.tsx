...
const StudentResult = () => {
  ...
  const sortedResults = [...filteredResults].sort((a, b) => {
    ...
    const [aValue, bValue] = sortConfig.key.split(".").reduce((obj, key) => [obj[0][key], obj[1][key]], [a, b])
      ...
      <div className="bg-white p-4 rounded-lg shadow-lg border border-gray-300 overflow-x-auto">
        <div className="relative max-w-xs mb-3 float-right">
          ...
          <input
            ...
            className="p-3 pl-10 w-full border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 text-gray-900 bg-gray-50 hover:bg-gray-100 transition duration-300 ease-in-out"
          />
        </div>
        ...
        <table className="w-full border-collapse min-w-[600px] border border-gray-200">
          <thead>
            <tr className="bg-gray-800 text-white">
              {["student.name", "exam.name", "marks", "cgpa", "result_Date", "status", "faculty.name"].map((column) => (
                <th
                  key={column}
          ...
          <tbody>
            {sortedResults.length > 0 ? (
              sortedResults.map((result: any, index: number) => (
                ...
                >
                  <td className="border border-gray-300 p-3 text-gray-900 font-semibold">{result.student.name}</td>
                  <td className="border border-gray-300 p-3 text-gray-800">{result.exam.subject}</td>
                  <td className="border border-gray-300 p-3 text-gray-800">{result.marks}</td>
                  <td className="border border-gray-300 p-3 text-gray-800">{result.cgpa}</td>
                  ...
                  <td className="border border-gray-300 p-3 text-gray-800">{result.faculties?.name || "N/A"}</td>
                </tr>
              ))
            ) : (
              <tr>
